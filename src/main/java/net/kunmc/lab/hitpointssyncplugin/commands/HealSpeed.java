...
public class HealSpeed
{
    public static void healspeed(CommandSender sender, String[] args)
    {
        ...
        {
            case 0:
                sender.sendMessage(net.md_5.bungee.api.ChatColor.RED + "エラー：使用法が間違っています。使用法：/hpsync healspeed [チーム名] <回復HP(ハート数)>") ;
                return;
            case 1:
                Integer num = Utils.parseInt(args[0]);
                if (num == null)
                    sender.sendMessage(net.md_5.bungee.api.ChatColor.RED + "エラー：引数が数字ではありません。");
                else
                {
                    HitPointsSyncPlugin.managers.get("main").setRegenPerMinute(num);
                    sender.sendMessage(net.md_5.bungee.api.ChatColor.GREEN + "メインチームの回復レートを " + num + " にセットしました。");
                }
                return;
            case 2:
                ...
                else
                {
                    if (args[0].equals("*") || args[0].equals("all"))
                    {
                        ...
                        sender.sendMessage(net.md_5.bungee.api.ChatColor.GREEN + "全てのチームの回復レートを " + num + " にセットしました。");
                        return;
                    }

                    if (!HitPointsSyncPlugin.managers.containsKey(args[0]))
                    {
                        sender.sendMessage(net.md_5.bungee.api.ChatColor.RED + "エラー：チームが見つかりませんでした。");
                        return;
                    }
